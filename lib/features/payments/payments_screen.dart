...
class PaymentsScreen extends ConsumerStatefulWidget {
  final String view;
  const PaymentsScreen({super.key, this.view = 'daily'});
    ...
    return Scaffold(
      ...
      body: Column(
          ...
          children: [
            ...
            Builder(
              builder: (context) {
                final isDailyTab = widget.view == 'daily';
  ...
  Widget _buildPaymentsTable(
    ...
    for (final entry in groupedPayments.entries) {
      ...
      if (group.length == 1) {
          ...
          DataRow(
            cells: [
              DataCell(
                Text(
                  DateFormat(
                    isSmallScreen ? 'dd MMM yy' : 'dd MMM yyyy, hh:mm a',
                  ).format(dt),
                ),
              ),
        ...
        // Parent Row
        final dt = DateTime.parse(
          group.first.paymentDate ?? DateTime.now().toString(),
        );
        final totalAmount = group.fold(0.0, (sum, p) => sum + p.amount);
        final allModes = group
            .expand((p) => p.paymentModes)
            .toSet()
            .map((m) => m.name)
            .join(', ');
          ...
          DataRow(
            ...
            cells: [
              DataCell(
                Text(
                  DateFormat(
                    isSmallScreen ? 'dd MMM yy' : 'dd MMM yyyy',
                  ).format(dt),
        ...
        if (isExpanded) {
          for (final payment in group) {
            final dtChild = DateTime.parse(
              payment.paymentDate ?? DateTime.now().toString(),
            );
            final modesChild = payment.paymentModes
                .map((m) => m.name)
                .join(', ');
            tableRows.add(
              DataRow(
                ...
                cells: [
                  DataCell(
                    Padding(
                      ...
                      child: Text(
                        DateFormat(
                          isSmallScreen ? 'hh:mm a' : 'dd MMM yyyy, hh:mm a',
                        ).format(dtChild),
              ...
              child: SingleChildScrollView(
                child: DataTable(
                  headingRowColor: WidgetStateProperty.resolveWith(
                    (states) => primaryColor.withValues(alpha: 0.03),
                  ),
